/* blktiger_audio.c -- Black Tiger sound: audio Z80 (bd-06.1l) + 2x YM2203.
 * Capcom's standard 2xYM2203 sound board -- the SAME hardware as 1943/Gun.Smoke,
 * so this is c1943_audio.c adapted to Black Tiger's clocks + sound map. Uses the
 ...
 * audio-delegation hooks (context == bt_audio_z).
 *
 * Sound Z80 map (MAME capcom/blktiger.cpp sound_map):
 *   0x0000-0x7fff ROM (bd-06.1l, 32K), 0xc000-0xc7ff RAM, 0xc800 soundlatch (read),
 *   YM2203 #1 at 0xe000/0xe001 (addr/data), YM2203 #2 at 0xe002/0xe003.
...
#include "driver.h"
#include "fm.h"
#include "blktiger_audio.h"
#include <string.h>

#define AUD_CLOCK         3579545
#define AUD_CYC_PER_FRAME 59659      /* 3.579545MHz sound Z80 / 60 Hz arcade frame */
...
#define YM_CLOCK          3579545    /* YM2203 master clock (both chips) */
#define SR                7920       /* 132 samples at 60Hz; close to Paula period 448 */
#define AY_TICKS_PER_SMP  ((YM_CLOCK/8)/SR)

/* machine audio-delegation hooks (blktiger_machine.c routes the audio CPU here) */
extern MY_LITTLE_Z80 *bt_audio_z;
extern void          (*bt_audio_wr_hook)(MY_LITTLE_Z80 *z, unsigned a, unsigned char v);
extern unsigned char (*bt_audio_rd_hook)(MY_LITTLE_Z80 *z, unsigned a);
extern void          (*bt_latch_hook)(unsigned char v);

/* optional host-debug trace of YM register writes (NULL in the Amiga build) */
void (*bt_audio_ym_trace)(int chip, int ad, unsigned char v) = 0;
/* optional host-debug trace of soundlatch commands posted by the main CPU */
void (*bt_latch_trace)(unsigned char v) = 0;
...
static void latch_push(unsigned char v){
    if(bt_latch_trace) bt_latch_trace(v);
    if(v==0xff) return;
    int n=(latw+1)&63;
...
unsigned bt_audio_pc(void){ return aud.state.pc & 0xffff; }
...
static unsigned char aud_rd(MY_LITTLE_Z80 *z, unsigned a){
    a &= 0xffff;
    if (a == 0xc800) return cur_latch;
    if (a >= 0xe000 && a <= 0xe003)
...
static void aud_wr(MY_LITTLE_Z80 *z, unsigned a, unsigned char v){
    ...
    if (!ym_inited) return;
    switch (a){
        case 0xe000: if(bt_audio_ym_trace)bt_audio_ym_trace(0,0,v); YM2203Write(0,0,v); break;
        case 0xe001: if(bt_audio_ym_trace)bt_audio_ym_trace(0,1,v); YM2203Write(0,1,v); break;
        case 0xe002: if(bt_audio_ym_trace)bt_audio_ym_trace(1,0,v); YM2203Write(1,0,v); break;  /* YM1 address */
        case 0xe003: if(bt_audio_ym_trace)bt_audio_ym_trace(1,1,v); YM2203Write(1,1,v); break;  /* YM1 data    */
    }
}
...
void bt_audio_init(const unsigned char *snd)
{
    ...
    bt_audio_z       = &aud;
    bt_audio_wr_hook = aud_wr;
    bt_audio_rd_hook = aud_rd;
    bt_latch_hook    = latch_push;
}
...
void bt_audio_shutdown(void){ if(ym_inited){ YM2203Shutdown(); ym_inited=0; } }
...
void bt_audio_frame(signed char *out, int nsamples){
    bt_audio_render_samples(out, nsamples);
}
