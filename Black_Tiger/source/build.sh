#!/bin/bash
# Black Tiger RTG build (m68k-amigaos). Shared engine (main Z80 + machine +
# software renderer) -> fixed RGB332 RTG presenter with embedded Bezel Project
# backdrop and play-window-only refresh. No-MCU bootleg ROM set.
set -e
...
echo "== embed Black Tiger RTG bezel =="
python3 make_blktiger_rtg_bezel.py >/dev/null

echo "== compile engine + presenter =="
$GCC -DZ80_MAP_BLKTIGER -c z80.c -o "$B/z80.o"
$GCC -c blktiger_machine.c  -o "$B/blktiger_machine.o"
$GCC -c blktiger_render.c   -o "$B/blktiger_render.o"
$GCC -c blktiger_aga_main.c -o "$B/blktiger_aga_main.o"
$GCC -c "$GAP/gaplus_menu.c" -o "$B/gaplus_menu.o"
$GCC -c "$AI/arcade_intro.c" -o "$B/arcade_intro.o"

echo "== compile sound (YM2203 OPN core + sound Z80 + Paula) =="
YMDEF="-DHAS_YM2203=1 -DHAS_YM2608=0 -DHAS_YM2610=0 -DHAS_YM2610B=0 -DHAS_YM2612=0 -DHAS_YM3438=0"
# Audio at -O1 -fno-strict-aliasing: host AND big-endian-qemu both play perfect music
# (byte-identical PCM), so the chain is correct -- the m68k-gcc -O2 codegen was the only
# untested variable + miscompiled the timer/IRQ path. -O1 + no-aliasing avoids it.
...
$GCC_AUD $YMDEF -I ym -c blktiger_audio.c -o "$B/blktiger_audio.o"
$GCC_AUD -c blktiger_audio_amiga.c        -o "$B/blktiger_audio_amiga.o"

LIBGCC="obj_interp/libgcc_extract"
...
if [ "$LIBGCC" != "$B/libgcc_extract" ]; then
  cp "$LIBGCC/_udivdi3.o" "$B/libgcc_extract/_udivdi3.o"
  cp "$LIBGCC/_umoddi3.o" "$B/libgcc_extract/_umoddi3.o"
fi

echo "== assemble entry + embedded ROMs (vasm) =="
$VASM -o "$B/slave.o"       slave.s
$VASM -o "$B/amiga.o"       amiga.s
$VASM -o "$B/hal_sysvars.o" hal_sysvars.s
$VASM -o "$B/pl_support.o"  pl_support.s
$VASM -o "$B/romdata.o"     blktiger_romdata.s
$VASM -I build/bezel -o "$B/rtg_bezeldata.o" blktiger_rtg_bezeldata.s

echo "== assemble ArcadeIntro ptplayer + glue + music =="
...
vlink -b amigahunk -Bstatic -Cexestack -mrel -sc \
    ...
    "$B/blktiger_aga_main.o" "$B/blktiger_render.o" "$B/blktiger_machine.o" \
    "$B/gaplus_menu.o" \
    "$B/blktiger_audio.o" "$B/blktiger_audio_amiga.o" "$B/fm.o" \
    "$B/arcade_intro.o" "$B/arcade_intro_glue.o" "$B/tc_ptplayer.o" "$B/tc_ptplayer_glue.o" "$B/intro_mod.o" \
    "$B/z80.o" "$B/romdata.o" "$B/rtg_bezeldata.o" \
    "$B/libgcc_extract/_udivdi3.o" "$B/libgcc_extract/_umoddi3.o"
ls -la blktiger_aga | awk '{print "exe:",$5,"bytes"}'
cp -f blktiger_aga "../Black Tiger"
ls -la "../Black Tiger"

echo "== build direct-boot RTG HDF =="
HDF="$DIST/BlackTiger_RTG.hdf"
UAE="$DIST/BlackTiger-RTG.uae"
...
if [ -f "$RTG_BASE" ]; then
  ...
  if xdftool "$RTG_BASE" read S/startup-sequence "$B/base-startup-sequence" >/dev/null 2>&1; then
    sed -e 's/Commando RTG/Black Tiger RTG/g' \
        -e 's/Gun.Smoke RTG/Black Tiger RTG/g' \
        -e 's/Side Arms RTG/Black Tiger RTG/g' \
        -e 's/SYS:Commando/SYS:BlackTiger/g' \
        -e 's/SYS:gunsmoke/SYS:BlackTiger/g' \
        -e 's/SYS:SideArms/SYS:BlackTiger/g' \
        "$B/base-startup-sequence" > "$B/startup-sequence"
  else
    printf 'Echo "Black Tiger RTG"\nStack 65536\nSYS:BlackTiger\nC:UAEquit\nEndCLI >NIL:\n' > "$B/startup-sequence"
  fi
  ...
  xdftool "$HDF" write "blktiger_aga" BlackTiger \
      + write "../Black Tiger.info" BlackTiger.info \
      + write "$B/startup-sequence" S/startup-sequence
else
  printf 'Echo "Black Tiger RTG"\nStack 65536\nSYS:BlackTiger\nC:UAEquit\nEndCLI >NIL:\n' > "$B/startup-sequence"
  normalize_rtg_startup "$B/startup-sequence"
  xdftool "$HDF" create size=8M \
      + format BLKTIGRTG ffs \
      + boot install \
      + write "blktiger_aga" BlackTiger \
      + write "../Black Tiger.info" BlackTiger.info \
      + makedir S \
      + write "$B/startup-sequence" S/startup-sequence
fi
...
cat > "$UAE" <<EOF
; Black Tiger RTG interpreted-Z80 config for Amiberry.

[config]
config_description=Black Tiger RTG INTERPRETER
...
nr_floppies=0
floppy0type=-1
hardfile2=rw,DH0:${RTG_BASE_DIR:-/home/jon/Amiberry/HardDrives}/BlackTiger_RTG.hdf,32,1,2,512,0,,uae0,0
uaehf0=hdf,rw,DH0:${RTG_BASE_DIR:-/home/jon/Amiberry/HardDrives}/BlackTiger_RTG.hdf,32,1,2,512,0,,uae0,0
gfx_display=0
gfx_display_rtg=0
gfx_width=864
gfx_height=486
...
log_file=/tmp/amiberry-blacktiger-rtg.log
log_console=1
fpu_model=68882
EOF
ls -la "$HDF" "$UAE"
