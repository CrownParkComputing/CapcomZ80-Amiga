...
/* pacland-amiga: route data reads/writes through the machine layer so
 * Galaxian memory-mapped I/O (inputs, gfx-bank/NMI/stars/flip control
 * writes, and the read/write overlap) is trapped. Opcode FETCH stays a
 * direct array access -- code only ever lives in ROM/RAM, never I/O. */
extern unsigned char machine_rd(MY_LITTLE_Z80 *z, unsigned int addr);
extern void          machine_wr(MY_LITTLE_Z80 *z, unsigned int addr, unsigned char val);

/* Optional per-instruction PC trace for the disassembler coverage tool
 * (tools/z80trace_host.c builds with -DZ80_TRACE_PC). Zero cost when off. */
#ifdef Z80_TRACE_PC
extern void z80_trace_pc(int pc);
#endif

/* Fast path: ROM/RAM/VRAM read straight from the array; only memory-mapped
 * I/O needs the machine_rd trap. Avoids a function call on the ~99% of
 * accesses that aren't I/O.
 *
 * The default predicate (<0xa000) suits a Galaxian-style map (Moon Cresta);
 * games whose RAM/VRAM/banked-ROM live higher (e.g. 1943: code banks to
 * 0xbfff, VRAM 0xd000, RAM 0xe000, spriteram 0xf000, only 0xc000-0xc7ff is
 * real I/O) override Z80_FAST_READ / Z80_FAST_WRITE at build time so those
 * hot regions skip the function-call trap entirely. */
#if defined(Z80_MAP_1943) && !defined(Z80_FAST_READ)
/* 1943 memory map for z80.c's inline fast path. NOTE: z80.c is shared by BOTH
 * 1943 CPUs (main + audio), so the predicates must be correct for both.
 *
 *   Main: 0x0000-0xbfff fixed+banked ROM, 0xc000-0xc7ff I/O (inputs/protection),
 *         VRAM/colorram 0xd000-0xd7ff, RAM 0xe000-0xefff, spriteram 0xf000-0xffff.
 *   Audio: 0x0000-0x7fff ROM, 0xc000-0xc7ff RAM, 0xc800 soundlatch,
 *          YM2203 regs at 0xe000-0xe003.
 *
 * Reads: everything is plain memory[] except the 0xc000-0xcfff I/O page, which
 * must trap -- main inputs (0xc000-0xc007) AND the audio soundlatch read at
 * 0xc800. (Trapping the whole page is identical for the main CPU: machine_rd
 * returns memory[a] for the unhandled addresses anyway.) Writes: hit memory[]
 * directly for VRAM/colorram (0xd000-0xd7ff) and main RAM/spriteram (0xe004+);
 * trap for control/scroll ports, ROM, AND 0xe000-0xe003 -- those four are the
 * audio YM2203 registers and MUST reach machine_wr (writing them to RAM would
 * silently kill all sound). Trapping 0xe000-0xe003 costs the main CPU nothing
 * measurable (4 of 4K RAM bytes; machine_wr still stores them for the main
 * context). */
#define Z80_FAST_READ(a)  (((a) & 0xf000u) != 0xc000u)
#define Z80_FAST_WRITE(a) (((a) >= 0xd000u && (a) <= 0xd7ffu) || (a) >= 0xe004u)
#endif
#if defined(Z80_MAP_GUNSMOKE) && !defined(Z80_FAST_READ)
/* Gun.Smoke memory map for z80.c's inline fast path (shared by BOTH CPUs).
 * Identical layout to 1943 (same Capcom sound/main board):
 *   Main:  0x0000-0xbfff fixed+banked ROM, 0xc000-0xc4cb I/O (inputs/protection),
 *          VRAM/colorram 0xd000-0xd7ff, scroll/layer 0xd800-0xd806, RAM
 *          0xe000-0xefff, spriteram 0xf000-0xffff.
 *   Audio: 0x0000-0x7fff ROM, 0xc000-0xc7ff RAM, 0xc800 soundlatch (read),
 *          YM2203 regs at 0xe000-0xe003.
 * Reads: trap the whole 0xc000 page (main inputs/protection + audio latch).
 * Writes: plain for VRAM/colorram (0xd000-0xd7ff) and RAM/spriteram (0xe004+);
 * trap control/scroll ports, the soundlatch, ROM, AND 0xe000-0xe003 (the audio
 * YM2203 registers -- writing them to RAM would silently kill all sound). */
#define Z80_FAST_READ(a)  (((a) & 0xf000u) != 0xc000u)
#define Z80_FAST_WRITE(a) (((a) >= 0xd000u && (a) <= 0xd7ffu) || (a) >= 0xe004u)
#endif
#if defined(Z80_MAP_COMMANDO) && !defined(Z80_FAST_READ)
/* Commando memory map for z80.c's inline fast path (shared by BOTH CPUs).
 *   Main:  0x0000-0xbfff ROM, 0xc000-0xc004 inputs, 0xc800-0xc80b I/O (latch/
 *          control/sprite-DMA/scroll), VRAM 0xd000-0xdfff, RAM 0xe000-0xffff.
 *   Audio: 0x0000-0x3fff ROM, 0x4000-0x47ff RAM, 0x6000 soundlatch (read),
 *          YM2203 regs at 0x8000-0x8003.
 * CRUCIAL: the audio YM regs sit at 0x8000-0x8003 -- the DEFAULT fast-write range
 * (0x8000-0x9fff) would silently send them to memory[] and KILL all sound. So we
 * must trap that range. Reads: trap the 0xc000 page (main inputs) and the 0x6000
 * page (audio latch); all else plain. Writes: fast only for the pure RAM/VRAM
 * regions of either CPU (audio RAM 0x4000-0x47ff, main VRAM+RAM 0xd000-0xffff);
 * everything else (incl. audio YM 0x8000-0x8003 and main I/O 0xc800-0xc80b) traps. */
#define Z80_FAST_READ(a)  ((((a) & 0xf000u) != 0xc000u) && (((a) & 0xf000u) != 0x6000u))
#define Z80_FAST_WRITE(a) (((a) >= 0xd000u) || (((a) >= 0x4000u) && ((a) < 0x4800u)))
#endif
#if defined(Z80_MAP_1942) && !defined(Z80_FAST_READ)
/* 1942 memory map for z80.c's inline fast path (shared by BOTH CPUs).
 *   Main:  0x0000-0xbfff fixed+banked ROM, 0xc000-0xc806 inputs/control,
 *          0xcc00-0xcc7f spriteram, 0xd000-0xdbff VRAM, 0xe000-0xefff RAM.
 *   Audio: 0x0000-0x3fff ROM, 0x4000-0x47ff RAM, 0x6000 soundlatch (read),
 *          AY ports at 0x8000/1 and 0xc000/1.
 * Writes must trap both AY port ranges and the main 0xc800 control block, while
 * plain RAM/VRAM stays direct. */
#define Z80_FAST_READ(a)  (((a) & 0xf000u) != 0xc000u)
#define Z80_FAST_WRITE(a) (((a) >= 0xcc00u) || (((a) >= 0x4000u) && ((a) < 0x4800u)))
#endif
#if defined(Z80_MAP_BLKTIGER) && !defined(Z80_FAST_READ)
/* Black Tiger memory map for z80.c's inline fast path. z80.c is shared by BOTH
 * Black Tiger CPUs (main + sound), so the predicates must be correct for both.
 *   Main:  0x0000-0xbfff fixed+banked ROM, 0xc000-0xcfff BANKED bg-video-RAM
 *          window (must trap: machine_rd redirects to scrollram[...]), text
 *          0xd000-0xd7ff, palette 0xd800-0xdfff, work RAM 0xe000-0xfdff,
 *          spriteram 0xfe00-0xffff. I/O is PORT-mapped (IN/OUT->in/out_impl).
 *   Sound: 0x0000-0x7fff ROM, 0xc000-0xc7ff RAM, 0xc800 soundlatch (read),
 *          YM2203 regs at 0xe000-0xe003.
 * Reads: trap ONLY the 0xc000 window -- covers the main bg-RAM window AND the
 * sound soundlatch read at 0xc800. Writes: plain for text/palette (0xd000-
 * 0xdfff) and main RAM/spriteram (0xe004+); trap below 0xd000 (ROM ignored, bg
 * window redirected, sound RAM) AND 0xe000-0xe003 -- those four are the sound
 * YM2203 registers and MUST reach machine_wr (writing them to RAM would silently
 * kill all sound). Trapping 0xe000-0xe003 costs the main CPU nothing measurable
 * (4 of its work-RAM bytes; machine_wr still stores them for the main context).
 * Reads ALSO trap 0xe000-0xe003: the sound CPU's RST38 handler reads the YM2203
 * status (0xe000) to test the timer-overflow flags, so those reads must reach the
 * OPN core (machine_rd -> YM2203Read); the main CPU's reads of those 4 work-RAM
 * bytes still return memory[] via machine_rd's fall-through. */
#define Z80_FAST_READ(a)  ((((a) & 0xf000u) != 0xc000u) && ((a) < 0xe000u || (a) > 0xe003u))
#define Z80_FAST_WRITE(a) (((a) >= 0xd000u && (a) < 0xe000u) || (a) >= 0xe004u)
#endif
#ifndef Z80_FAST_READ
#define Z80_FAST_READ(a)  ((a) < 0xa000)
#endif
...
/* M1 opcode fetch. Identical to Z80_FETCH_BYTE unless the CPU has a decrypted
 * opcode shadow installed (opcode-encrypted sets); operands/displacements keep
 * using Z80_FETCH_BYTE so they stay plain. NULL shadow => same as plain fetch. */
#define Z80_FETCH_OPCODE(address, x)                                    \
{                                                                       \
        MY_LITTLE_Z80 *_z = (MY_LITTLE_Z80 *)context;                   \
        unsigned int _oa = (address) & 0xffff;                          \
        (x) = (_z->opcodes && _oa < _z->opcodes_len)                    \
...
{                                                                       \
        ...
        _lo = Z80_FAST_READ(_a) ? ((MY_LITTLE_Z80 *)context)->memory[_a]\
                            : machine_rd((MY_LITTLE_Z80 *)context, _a);  \
        _a = (_a + 1) & 0xffff;                                         \
        _hi = Z80_FAST_READ(_a) ? ((MY_LITTLE_Z80 *)context)->memory[_a]\
                            : machine_rd((MY_LITTLE_Z80 *)context, _a);  \
        (x) = _lo | (_hi << 8);                                         \
}
...
{                                                                       \
        unsigned char *memory = ((MY_LITTLE_Z80 *)context)->memory;     \
        (x) = memory[(address) & 0xffff]                                \
            | (memory[((address) + 1) & 0xffff] << 8);                  \
}

/* Fast path: RAM/VRAM/objram (0x8000-0x9fff) write straight to the
 * array; ROM and I/O go through machine_wr (which ignores ROM writes
 * and decodes the control ports). */
#ifdef WR_LOG_HOOK
extern void wr_log_hook(unsigned int a, unsigned char v, unsigned int pc);
static unsigned int z80_wr_current_pc;
#define WRLOG(C,_a,_x) wr_log_hook((_a),(unsigned char)(_x),z80_wr_current_pc)
#else
#define WRLOG(C,_a,_x)
#endif
#define Z80_WRITE_BYTE(address, x)                                      \
{                                                                       \
        unsigned int _a = (address) & 0xffff;                           \
        WRLOG(context,_a,(x));                                          \
        if (Z80_FAST_WRITE(_a))                                         \
            ((MY_LITTLE_Z80 *)context)->memory[_a] = (x);               \
        else                                                            \
            machine_wr((MY_LITTLE_Z80 *)context, _a, (x));              \
}

#define Z80_WRITE_WORD(address, x)                                      \
{                                                                       \
        unsigned int _a = (address) & 0xffff, _v = (x);                 \
        WRLOG(context,_a,_v);                                           \
        if (Z80_FAST_WRITE(_a))                                         \
            ((MY_LITTLE_Z80 *)context)->memory[_a] = _v;                \
        else machine_wr((MY_LITTLE_Z80 *)context, _a, _v);              \
        _a = (_a + 1) & 0xffff;                                        \
        WRLOG(context,_a,_v >> 8);                                      \
        if (Z80_FAST_WRITE(_a))                                         \
            ((MY_LITTLE_Z80 *)context)->memory[_a] = _v >> 8;           \
        else machine_wr((MY_LITTLE_Z80 *)context, _a, _v >> 8);         \
}
...
int Z80Emulate (Z80_STATE *state, int number_cycles, void *context)
{
	...
	pc = state->pc;
#ifdef WR_LOG_HOOK
        z80_wr_current_pc = pc & 0xffff;
#endif
        Z80_FETCH_OPCODE(pc, opcode);
        state->pc = pc + 1;
				...
				if ((Y(opcode) & 0x03) <= 0x01)
                        ...
                        case CB_PREFIX: {
                                ...
                                if (registers != state->register_table) {
                                         ...
                                         * correctly update pc.
                                         *
                                         * The 4th byte of a DD/FD CB d op
                                         * instruction is the op byte, which the
                                         * real Z80 reads as a NON-M1 operand
                                         * fetch (after the displacement). On an
                                         * opcode-encrypted CPU (Commando: M1
                                         * fetches read the decrypted shadow) it
                                         * must therefore be read PLAIN, not via
                                         * Z80_FETCH_OPCODE -- decrypting it
                                         * selects the wrong CB op (e.g. the
                                         * register-form BIT instead of the
                                         * (IX+d) form), which both does the
                                         * wrong thing AND fails to consume the
                                         * displacement byte, derailing PC into
                                         * data -> spurious RST/reset. For
                                         * unencrypted CPUs (opcodes==NULL)
                                         * Z80_FETCH_BYTE == the old fetch. */
