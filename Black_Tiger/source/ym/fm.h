...
/* This is an FP-free fm.c: its INTERNAL timer is non-functional (freqbase is never
 * set -> the INTERNAL_TIMER macros never decrement). Black Tiger's sound driver
 * GATES music processing on the YM2203 timer-A/B overflow flags, so we drive the
 * timers EXTERNALLY from blktiger_audio.c (integer sample counters -> YM2203TimerOver,
 * which sets the status flags + raises the IRQ). Keep external-timer mode. */
