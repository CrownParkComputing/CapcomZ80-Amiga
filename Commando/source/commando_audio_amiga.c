/* src/hal/commando_audio_amiga.c -- Amiga Paula playback for the Commando sound
 * chain. Uses the same lead-buffer ring approach as Black Tiger so F10/DIP pauses
 * can stop and restart Paula cleanly without leaving a stale looping frame. */
#include <exec/exec.h>
...
extern void commando_audio_render_samples(signed char *out, int nsamples);
...
#define C_SR     8040
#define C_PER    441           /* PAL 3546895 / 8040 */
...
static void ring_render(unsigned long n)
{
    while(n){
        ...
        if(chunk > n) chunk = n;
        commando_audio_render_samples(ring + pos, (int)chunk);
        p_wrote += chunk;
        n -= chunk;
    }
}

void commando_audio_amiga_close(void);

void commando_audio_amiga_open(void)
{
    volatile uint16_t *c = CUSTOM;
    if(ring) commando_audio_amiga_close();
...
void commando_audio_amiga_frame(void)
{
