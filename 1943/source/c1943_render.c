                       ...
                       const uint8_t *tilerom, const uint8_t *proms)
{
    ...
    if (!dec_char || !dec_char_ink || !dec_bg1 || !dec_bg2 || !dec_spr) {
        c1943_render_shutdown();
        return;
    }
    ...
    for (int c = 0; c < NBG1;  c++) for (int y = 0; y < 32; y++) for (int x = 0; x < 32; x++)
        DEC_BG1(c)[y*32 + x]  = (uint8_t)bg1_pix(c, x, y);
    for (int c = 0; c < NBG2;  c++) for (int y = 0; y < 32; y++) for (int x = 0; x < 32; x++)
...
void c1943_render_frame(MY_LITTLE_Z80 *z, uint8_t fb[C1943_NH][C1943_NW])
{
    ...
    const int obj_on =
        ...
        c1943_obj_on();
#endif
    const int char_on = c1943_char_on();
    ...
    for (int oy = 0; oy < C1943_NH; oy++) {
        ...
        if (bg2_on) {
            ...
            while (ox < C1943_NW) {
                ...
                const uint8_t *trow = DEC_BG2(code) + ((attr & 0x80) ? 31 - iyr : iyr) * 32;
                const uint8_t *cb = bg2_ctab + ((attr & 0x3c) >> 2) * 16;
                int fx = attr & 0x40;
                for (; sub < 32 && ox < C1943_NW; sub++, ox++) {
                    fbrow[ox] = cb[trow[fx ? 31 - sub : sub]];
                    prow[ox] = 1;
                }
            }
        } else {
        ...
        if (bg1_on && runtime_parallax) {
            ...
            while (ox < C1943_NW) {
                ...
                const uint8_t *trow = DEC_BG1(code) + ((attr & 0x80) ? 31 - iyr : iyr) * 32;
                int colr = (attr & 0x3c) >> 2, fx = attr & 0x40;
                for (; sub < 32 && ox < C1943_NW; sub++, ox++) {
                    int k = colr * 16 + trow[fx ? 31 - sub : sub];
    ...
    if (obj_on) {
        ...
        for (int offs = 0; offs < 0x1000; offs += 32) {
            ...
            for (int j = 0; j < 16; j++) {
                ...
                int sj = fy ? 15 - j : j;
                const uint8_t *srow = DEC_SPR(code) + sj * 16;
                uint8_t *fbrow = fb[oy];
                uint8_t *prow = pri[oy];
                int kbase = color * 16;
                for (int X = x0; X < x1; X++) {
                    int i = X - sx0;
                    int rp = srow[fx ? 15 - i : i];
                    if (rp == 0) continue;
                    uint8_t pv = prow[X];
    ...
    if (char_on) {
        ...
        for (int oy = 0; oy < C1943_NH; oy++) {
            ...
            for (int cx = 0; cx < 32; cx++) {
                ...
                const uint8_t *trow = DEC_CHAR(code) + ciy * 8;
