#!/bin/bash
# 1943 Kai RTG interpreter-main build (m68k-amigaos).
# Matches the Gun.Smoke path: C Z80 interpreter for the main CPU, RTG presenter,
# and the prefixed sound-Z80 native transcode for the audio board.
set -e
...
EXE="$SRC/1943kai_native_96"
DIST="$ROOT/dist"
HDF="$DIST/1943Kai_RTG.hdf"
UAE="$DIST/1943Kai-RTG.uae"

GCC="m68k-amigaos-gcc -m68030 -noixemul -O3 -fomit-frame-pointer -funroll-loops -DNDEBUG -DC1943_KAI -DZ80_MAP_1943 -I . -I $AI"
GCC_AUD="m68k-amigaos-gcc -m68030 -noixemul -O1 -fno-strict-aliasing -fomit-frame-pointer -DNDEBUG -DC1943_KAI -DZ80_MAP_1943 -I . -I $AI"
...
mkdir -p "$B" "$B/rsobj_audio" "$DIST" "$ROOT/prebuilt"
...
echo "== compile interpreter main, renderer, RTG presenter, and audio =="
$GCC     -c z80.c                          -o "$B/z80.o"
$GCC     -c c1943_machine.c                -o "$B/c1943_machine.o"
...
vlink -b amigahunk -Bstatic -Cexestack -mrel -sc \
    ...
    "$B/c1943_rtg_main.o" "$B/c1943_glue.o" "$B/c1943_render.o" "$B/c1943_machine.o" "$B/z80.o" \
...
cp -f "$EXE" "$ROOT/1943 Kai"
cp -f "$ROOT/assets/1943 Kai.info" "$ROOT/1943 Kai.info"
cp -f "$EXE" "$ROOT/prebuilt/1943 Kai"
ls -la "$EXE" "$ROOT/1943 Kai"
...
; 1943 Kai RTG direct boot, based on the supplied RTG1/Picasso96 boot disk.
...
STARTUP
  ...
  xdftool "$HDF" delete 1943Kai >/dev/null 2>&1 || true
  xdftool "$HDF" delete 1943Kai.info >/dev/null 2>&1 || true
  xdftool "$HDF" delete "1943 Kai" >/dev/null 2>&1 || true
  xdftool "$HDF" delete "1943 Kai.info" >/dev/null 2>&1 || true
  xdftool "$HDF" write "$ROOT/1943 Kai" 1943Kai \
      + write "$ROOT/1943 Kai.info" 1943Kai.info \
...
cat > "$UAE" <<EOF
; 1943 Kai RTG interpreter-main config.
; Boots the RTG1/Picasso96-derived HDF directly into the game.

[config]
config_description=1943 KAI RTG INTERPRETER
...
hardfile2=rw,DH0:${RTG_BASE_DIR:-/home/jon/Amiberry/HardDrives}/1943Kai_RTG.hdf,32,1,2,512,0,,uae0,0
uaehf0=hdf,rw,DH0:${RTG_BASE_DIR:-/home/jon/Amiberry/HardDrives}/1943Kai_RTG.hdf,32,1,2,512,0,,uae0,0
...
log_file=/tmp/amiberry-1943kai-rtg.log
log_console=1
fpu_model=68882
EOF
