...
extern int      Z80Emulate (Z80_STATE *state, 
			int number_cycles, 
			void *context);
extern void     Z80BuildQuick (struct MY_LITTLE_Z80 *z,
			unsigned int start,
			unsigned int len);
...
typedef struct MY_LITTLE_Z80 {
	...
	/* Optional decrypted-OPCODE shadow for opcode-encrypted CPUs (e.g. Capcom
	 * Commando: M1 opcode fetch reads the bitswapped image, operands/data stay
	 * plain). NULL => fetch from memory[] (unencrypted; 1942/1943 path). Covers
	 * the ROM region; opcode fetches at/above opcodes_len fall back to memory[]. */
	const unsigned char *opcodes;
	unsigned int         opcodes_len;
	unsigned char        quick_opcode[1 << 16];
	unsigned char        quick_instruction[1 << 16];
	unsigned char        quick_valid[1 << 16];
	unsigned char        quick_enabled;
} MY_LITTLE_Z80;
/*
