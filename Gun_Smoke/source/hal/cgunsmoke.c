/* src/hal/cgunsmoke.c -- Gun.Smoke (Capcom, 1985) machine, MAIN Z80.
...
/* audio CPU delegation -- z80.c calls the same global machine_rd/wr for BOTH the
 * main and audio Z80, so we dispatch by context (z == gunsmoke_audio_z). The
 * audio subsystem (cgunsmoke_audio.c) installs these. Mirrors c1943.c. */
MY_LITTLE_Z80 *gunsmoke_audio_z = 0;
void          (*gunsmoke_audio_wr_hook)(MY_LITTLE_Z80 *z, unsigned a, unsigned char v) = 0;
unsigned char (*gunsmoke_audio_rd_hook)(MY_LITTLE_Z80 *z, unsigned a) = 0;
void          (*gunsmoke_latch_hook)(unsigned char v) = 0;
...
{
    a &= 0xffff;
    if (z == gunsmoke_audio_z) return gunsmoke_audio_rd_hook ? gunsmoke_audio_rd_hook(z, a) : z->memory[a];
...
{
    a &= 0xffff;
    if (z == gunsmoke_audio_z) { if (gunsmoke_audio_wr_hook) gunsmoke_audio_wr_hook(z, a, v); return; }
    if (a < 0xc000) return;                          /* ROM / bank window */
    switch (a) {
        case 0xc800: soundlatch = v; if (gunsmoke_latch_hook) gunsmoke_latch_hook(v); return;
        case 0xc804:
            control = v;
