...
int hwscroll_open(hwscroll_t *s, int nlayers, int pfplanes,
    ...
    { int hstart=0x81+(320-disp_w)/2+hwscroll_hwin_adj, ddf=(hstart-0x11)/2;
      int preroll = hwscroll_no_preroll ? 0 : 8;   /* extra fine-scroll fetch word (px<<-1) */
      /* Horizontal fine scroll shifts the playfield RIGHT by up to 15px, which pulls
       * in pixels from one word PAST the normal fetch window at the right edge. The
       * pre-roll word covers the LEFT; without a matching RIGHT slack word the last
       * 1-2 character columns drop out as soon as the fine nibble is non-zero. So
       * when fine scroll is in use (preroll), fetch ONE extra word on the right too;
       * DIWSTOP still clips the visible area to disp_w, so it isn't shown, it just
       * feeds the shifted-in pixels. Vertical scrollers (no preroll) are unchanged. */
    ...
    { int modadj = hwscroll_no_preroll ? 0 : 4;
...
void hwscroll_hud_bands(hwscroll_t *s, uint8_t *bg_bpl, uint8_t *hud_bpl,
    ...
    for (int p=0;p<s->pfplanes;p++){
        uint32_t a=(uint32_t)(hud_bpl + (long)p*hud_plane_sz + (long)vc*s->stride);
     ...
     * (V+vcrop+play_top); +vc keeps it crop-aligned with the top band. play_bot unused. */
    (void)play_bot;
    int w=s_hudsplit;
    w=hud_emit(s, w, bg_bpl, s->plane_sz, (long)(bg_v+vc+play_top)*s->stride, play_top);
    /* trailing words stay NOP (open) -> copper falls through to the end marker. */
}
...
void hwscroll_frame_lateok(hwscroll_t *s){
    volatile uint16_t *c=CUSTOM; if(!s->ok) return;
    volatile uint32_t *vp=(volatile uint32_t*)0xdff004; unsigned long g=0;
    uint32_t r=*vp; uint32_t v=(((r>>16)&1)<<8)|((r>>8)&0xff);
    if (v < 300) {
        for(;;){ r=*vp; v=(((r>>16)&1)<<8)|((r>>8)&0xff); if(v>=300)break; if(++g>600000UL)break; }
    }
    s->spr = s->spr_buf[0];
    { uint32_t a=(uint32_t)s->copper; c[R_COP1LCH]=(uint16_t)(a>>16); c[R_COP1LCH+1]=(uint16_t)(a&0xFFFF); }
    c[R_DMACON]=0x83E0;
    c[R_INTENA]=0x7FFF;
    if (hwscroll_intena_keep) c[R_INTENA]=hwscroll_intena_keep;
}
